   ...
   }else
   {
	...
	if(count($rows)>0)
	{
		foreach($rows as $row)
		{
			...
			echo "<td>
			<div class='form-group'><button type='submit' class='btn btn-success btn-block'>تم التعامل <i class='fa fa-check-square' aria-hidden='true'></i></i></button></div>
			<div class='form-group'><a href='sendnot.php?uid=".$row["U_id"]."&id=$id'><button type='button' class='btn btn-success btn-block'>
